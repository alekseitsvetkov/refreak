...
export default async () => {
  const { refreakV5Closed } = await storage.getAll()

  if (!refreakV5Closed) {
    createNotification({
      title: 'Switch to Refreak v5',
      description:
        "We've recently released Refreak v5 and you're still using v4. We won't release v5 on Firefox Add-ons, but instead provide a self-distributed add-on on our website, which allows us to release updates almost instantly and it's still reviewed and verified by Mozilla.",
      linkLabel: 'Get Refreak v5 now',
      link: 'https://refreak.gg',
      onClose: () => {
        storage.set({ refreakV5Closed: true })
      },
    })
  }
}

...
export default async () => {
  ...
  if (!config.skinOfTheMatch.enabled) {
    return
  }
  ...
  if (!skinOfTheMatch?.steamId) {
    return
  }

  const skinOfTheMatchPlayer = players.find(
    ({ gameId }) => gameId === skinOfTheMatch.steamId,
  )
  ...
  const statsEventData = {
    ...
    skin: skinOfTheMatch?.skinportUrl?.split('/').pop() || '',
  }

  const skinOfTheMatchElement = (
    <a
      href={`${skinOfTheMatch?.skinportUrl || '#'}?r=refreak&utm_source=refreak&utm_medium=browser_extension`}
      target="_blank"
      ...
      >
        ...
        <div className="flex justify-center gap-3">
          ...
          <div className="flex-1 flex justify-center flex-col gap-0.5 w-40">
            ...
            <div className="text-white text-xs font-bold">
              <div>{skinOfTheMatch?.type || ''}</div>
              <div>{skinOfTheMatch?.name || ''}</div>
            </div>
            <div>
              {skinOfTheMatch?.exterior || ''}
              {skinOfTheMatch?.exterior && ' · '}
              {skinOfTheMatch?.price &&
                new Intl.NumberFormat(undefined, {
                  style: 'currency',
                  currency: skinOfTheMatch.price.currency,
                }).format(skinOfTheMatch.price.amount)}
            </div>
          </div>
        </div>
        <div className="invisible absolute bottom-[calc(100%_+_0.5rem)] left-0 right-0 opacity-0 flex flex-col items-center transition group-hover:visible group-hover:opacity-100">
          <div className="rounded-md bg-neutral-950 max-w-64 p-2 text-center">
            This feature is provided by Refreak and not affiliated with FACEIT.
          </div>
